...
namespace utilities
{
	...
	uintptr_t patternScan(const char* mod, const char* mask)
	{
		auto m_mask = mask;
		uintptr_t first = 0;
		uintptr_t ranageStart = reinterpret_cast<uintptr_t>(LF(GetModuleHandleA)(mod));
		MODULEINFO modInfo;
		LF(K32GetModuleInformation)(LF(GetCurrentProcess)(), reinterpret_cast<HMODULE>(ranageStart), &modInfo, sizeof(MODULEINFO));
